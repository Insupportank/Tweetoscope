...
--tab --title=KafkaBroker_0 --hold --command="/bin/bash -c 'sleep 3s ; $KAFKA_HOME/bin/kafka-server-start.sh $KAFKA_HOME/config/server_0.properties ; /bin/bash'" \
--tab --title=KafkaBroker_1 --hold --command="/bin/bash -c 'sleep 3s ; $KAFKA_HOME/bin/kafka-server-start.sh $KAFKA_HOME/config/server_1.properties ; /bin/bash'" \
--tab --title=TopicsCreated --hold --command="/bin/bash -c 'sleep 5s ; $KAFKA_HOME/bin/kafka-topics.sh --create --bootstrap-server localhost:9092,localhost:9093 --replication-factor 1 --partitions 2 --topic tweets ; $KAFKA_HOME/bin/kafka-topics.sh --create --bootstrap-server localhost:9092,localhost:9093 --replication-factor 1 --partitions 2 --topic filtered_tweet ;  $KAFKA_HOME/bin/kafka-topics.sh --create --bootstrap-server localhost:9092,localhost:9093 --replication-factor 1 --partitions 2 --topic hashtags ; /bin/bash'" \
